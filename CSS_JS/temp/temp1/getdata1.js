	...
	phXy(ctx,len,wid);
		...
		for(var i=0;i<35;i++){
			ctx.beginPath();
   			...
   			ctx.lineTo( 300+(i+1)*Dx*5,55 );
					 ...
					 var Dt=Math.floor((displayarea.getEnd()-displayarea.getStart())/7);
	                     ...
	                     ctx.fillStyle='black'
			             ctx.fillText(displayarea.getStart()+Dt*(i+1),250+(i+1)*Dl,29)
				 }
			...
			ctx.fillText(displayarea.getChr(),20,40)
   	        ctx.fillText(displayarea.getStart(),305,27)
			ctx.textAlign='right';
			if(Math.round(displayarea.getEnd())/100000<=1){	
				ctx.fillText(displayarea.getEnd(),canvas.width,27)
			}
   	        else{
				ctx.fillText(displayarea.getEnd(),canvas.width,27)
			}
		...
		ctx.font='25px Century';
		ctx.fillStyle='black';
		ctx.fillText(rr[j],300+wid*j+(wid/5)*2,110);
...
function drawXy(){
 ...
 for(var i=0;i<35;i++){
  ...
  for(var i=0;i<6;i++){
   ...
   ctx.font = '25px Century'  
   ctx.fillStyle='black'
   ctx.fillText(displayarea.getStart()+Dt*(i+1),250+(i+1)*Dl,27)
      }
   
  ctx.font = '25px Century'  
  ctx.fillStyle='black'
  ctx.fillText(displayarea.getChr(),20,40)
  ctx.fillText(displayarea.getStart(),301,27)
  ctx.textAlign='right';
  if(Math.round(displayarea.getEnd())/100000<=1){ 
   ctx.fillText(displayarea.getEnd(),canvas.width,27)
   }
