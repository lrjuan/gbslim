...
function draweletrack(trackname,mode,group){
		...
		var last=(Math.round(a[0].getElementsByTagName('F')[0].childNodes[0].nodeValue)-displayarea.getStart())*wid+300+(end-start+1)*wid+150;
		var last1=last;
		for(var j=1;j<a.length;j++){
			if(a.length<=100 && trackname=='PtrGene'){
		...
		for(var j=0;j<a.length;j++){
			...
			if(a.length<=100 && trackname=='PtrGene'){
						...
						ctx.fillStyle='black';
						ctx.fillText(a[j].id,x+w,level*20+20);
						temp=level;
						...
						button.id=a[j].id;
						var xlabel=document.getElementById('divPtrGene').getBoundingClientRect().width*(x/canvas.width);
						var ylabel=level*13+3;
...
function drawvaltrack(trackname,mode,group){
  ...
  ctx.fillText(Math.round(maxNum),305,25);
     if(mode=='pack'){canvas.height=60;drawthermogram(ctx,Arr,canvas);}//热图
